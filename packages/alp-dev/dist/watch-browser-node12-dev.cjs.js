...
nightingale.addConfig({
  key: 'pobpack-utils',
  handler: new ConsoleLogger__default(nightingale.levels.INFO)
});
...
const runDevServer = (compiler, port, proxyPort, options) => {
  const browserDevServer = new WebpackDevServer__default({
    ...
    static: {
      directory: path__default.resolve('public'),
      // use react-scripts ignoredFiles for perf
      watch: {
        ignored: ignoredFiles__default(path__default.resolve('src'))
      }
    },
    headers: {
    ...
    onBeforeSetupMiddleware(devServer) {
      ...
      devServer.app.use(evalSourceMapMiddleware__default(devServer));
    },
    ...
    onAfterSetupMiddleware(devServer) {
      ...
      devServer.app.use(noopServiceWorkerMiddleware__default('/'));
    },
...
runDevServer(browserCompiler, argv__default.port, argv__default['proxy-port']);
//# sourceMappingURL=watch-browser-node12-dev.cjs.js.map
