...
function _existsConfigSync(dirname, name) {
  ...
  return _flowRuntime2.default.param('dirname', _dirnameType).assert(dirname), _flowRuntime2.default.param('name', _nameType).assert(name), (0, _fs.existsSync)(`${dirname}${name}.json`);
}
...
function _loadConfigSync(dirname, name) {
  ...
  _flowRuntime2.default.param('dirname', _dirnameType2).assert(dirname), _flowRuntime2.default.param('name', _nameType2).assert(name);
...
let Config = exports.Config = (_dec = _flowRuntime2.default.decorate(_flowRuntime2.default.ref('Map', _flowRuntime2.default.string(), _flowRuntime2.default.any())), _dec2 = _flowRuntime2.default.decorate(_flowRuntime2.default.string()), _dec3 = _flowRuntime2.default.decorate(_flowRuntime2.default.object()), _class = class {
  ...
  constructor(dirname) {
    ...
    _flowRuntime2.default.param('dirname', _dirnameType3).assert(dirname), this._map = new Map(), this._dirname = dirname.replace(/\/*$/, '/');
  }
  ...
  loadSync(options = {}) {
    ...
    return socketPath ? config.set('socketPath', socketPath) : _minimistArgv2.default.port ? (config.set('port', _minimistArgv2.default.port), config.delete('socketPath')) : process.env.PORT && (config.set('port', Number(process.env.PORT)), config.delete('socketPath')), argvOverrides.forEach(key => {
    ...
    }), _returnType.assert(this._map = (0, _deepFreezeEs2.default)(config));
  }
  ...
  get(key) {
    ...
    return _flowRuntime2.default.param('key', _keyType).assert(key), _returnType2.assert(this._map.get(key));
  }
  ...
  existsConfigSync(name) {
    ...
    return _flowRuntime2.default.param('name', _nameType3).assert(name), _returnType3.assert(_existsConfigSync(this._dirname, name));
  }
  ...
  loadConfigSync(name) {
    ...
    return _flowRuntime2.default.param('name', _nameType4).assert(name), _returnType4.assert(_loadConfigSync(this._dirname, name));
  }
...
function alpConfig(dirname, options = {}) {
  ...
  return _flowRuntime2.default.param('dirname', _dirnameType4).assert(dirname), _flowRuntime2.default.param('options', ConfigOptions).assert(options), (app, config) => {
    let _configType = _flowRuntime2.default.nullable(_flowRuntime2.default.ref(Config));

    return _flowRuntime2.default.param('config', _configType).assert(config), config || (config = _configType.assert(new Config(dirname)), config.loadSync(options)), app.existsConfig = (0, _util.deprecate)(name => config.existsConfigSync(name), 'use app.existsConfigSync'), app.loadConfig = (0, _util.deprecate)(name => config.loadConfigSync(name), 'use app.loadConfigSync'), app.existsConfigSync = name => config.existsConfigSync(name), app.loadConfigSync = name => config.loadConfigSync(name), app.config = config, app.context.config = config, config;
  };
}
