...
export function subscribe(socket, name, callbackOnSubscribe, callbackOnUnsubscribe) {
  var diconnect = callbackOnUnsubscribe && function () {
    return callbackOnUnsubscribe();
  };
  socket.on('subscribe:' + name, function (callback) {
    logger.info('join', { name: name }), socket.join(name), callbackOnSubscribe ? callback(null, callbackOnSubscribe()) : callback(null), diconnect && socket.on('disconnect', diconnect);
  }), socket.on('unsubscribe:' + name, function (callback) {
    logger.info('leave', { name: name }), socket.leave(name), diconnect && socket.removeListener('disconnect', diconnect), callbackOnUnsubscribe ? callback(null, callbackOnUnsubscribe()) : callback(null);
  });
}
...
function start(config, dirname) {
  ...
  return logger.info('Starting', { port: port }), server.listen(port, function () {
