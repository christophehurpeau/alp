...
suite('test hello server', function () {
    ...
    test('hello without name', () => {
        return (0, _nodeFetch2.default)('http://localhost:5555/').then(res => res.text()).then(res => {
            (0, _assert.strictEqual)(...comparableContent(res, fixtures['hello-world']));
        });
    });

    test('hello with name', () => {
        return (0, _nodeFetch2.default)('http://localhost:5555/?name=Chris').then(res => res.text()).then(res => {
