...
let UserAccountsService = (_temp = _class = class extends EventEmitter {
  ...
  getScope(strategy, scopeKey, user, accountId) {
    ...
    t.param('strategy', _strategyType).assert(strategy), t.param('scopeKey', _scopeKeyType).assert(scopeKey), logger.debug('getScope', { strategy, userId: user && user._id });
  ...
  async findOrCreateFromGoogle(strategy, tokens, scope, subservice) {
    ...
    if (t.param('strategy', _strategyType2).assert(strategy), t.param('tokens', TokensObject).assert(tokens), t.param('scope', _scopeType).assert(scope), strategy !== 'google') throw new Error('Not supported at the moment');
